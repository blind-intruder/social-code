...
if (isset($_POST["data"])) {
  require("../../../../db-con/db.php");
	try{
        ...
        register_like(is_logged_in(),$reply_id);
	}
	catch(Exception $e){
		$res= array(
            'status' => '200 OK',
...
function check_comment_exits($reply_id){
  require("../../../../db-con/db.php");
  $sql="SELECT * from post_comment_reply where reply_id='$reply_id'";
...
function check_already_star($user_id,$reply_id){
  require("../../../../db-con/db.php");
...
function register_like($user_id,$reply_id){
  require("../../../../db-con/db.php");

  if(check_already_star($user_id,$reply_id)){//post already liked, so just dislike it
    $sql = "DELETE from post_reply_stars where reply_id='$reply_id' and user_id='$user_id'";
  ...
  if(check_comment_exits($reply_id)){//check if post exits
    $sql = "INSERT INTO post_reply_stars (reply_id,user_id) VALUES ('$reply_id','$user_id')";
    if (mysqli_query($conn, $sql)) {
      $res= array(
        'status' => '200 OK',
        'iscreated' => "true",
