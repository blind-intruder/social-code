...
function send_request($my_id,$uid)
{
  if(check_exits($my_id,$uid)){
    $res= array(
            'status' => '200 OK',
            'issent' => "true",
	...
	require("../../db-con/db.php");
	$sql = "INSERT into requests (sender,receiver) values ('$my_id','$uid')";
...
function check_exits($my_id,$uid){
  require("../../db-con/db.php");
...
{
  require("../../db-con/db.php");
  $sql = "DELETE from requests where sender='$my_id' and $uid='$uid'";
  $result = mysqli_query($conn, $sql);
  if (mysqli_query($conn, $sql)) {
    $res= array(
            'status' => '200 OK',
            'issent' => "true",
            'msg' => "friend request delete"
          );
          echo json_encode($res);
          die();
  }
